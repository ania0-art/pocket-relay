...
import type { IncomingMessage, OutgoingMessage, Task, AgentSessionInfo, ListSessionsOptions, PermissionRequest, ProgressUpdate } from '@pocket-relay/types';
...
export class Daemon {
  ...
  constructor(channel: IChannel, executor: IExecutor, cwd: string = process.cwd()) {
    ...
    this.taskQueue.setRunTaskCallback((task, executeOptions) =>
      this._runTask(task, executeOptions),
    );
  }
  ...
  async sendUnboundHint(chatId: string): Promise<void> {
    await this.send(chatId, `请先发送 /bind <node-id> 完成绑定。\n\nNode ID 在启动 PCR 时会打印在终端中。`);
  }
  ...
  private async _onIncomingMessage(msg: IncomingMessage): Promise<void> {
    ...
    const ctx: IDaemonCommandContext = {
      ...
      sessionManager: {
        ...
        setStatus: (chatId: string, status: any) =>
          this.sessionManager.setStatus(chatId, status),
      },
