...
import type { ChannelConfig, IncomingMessage, OutgoingMessage, InteractiveMessage, ProgressUpdate } from '@pocket-relay/types';
...
export class LarkChannel implements IChannel {
  ...
  async sendProgressUpdate(chatId: string, update: ProgressUpdate): Promise<void> {
    const icon = update.type === 'tool_call' ? '🔧'
      ...
      if (openId) {
        await this.client.im.v1.message.create({
          ...
          data: {
            receive_id: openId,
            msg_type: 'text',
            content: JSON.stringify({ text: '请向 PCR 进程发送 `/bind <node-id>` 完成绑定。\n\nNode ID 在启动 PCR 时会打印在终端中。' }),
          },
        });
      }
      return;
  ...
  private _trackMessageId(id: string): void {
    ...
    if (this.seenMessageIds.size > this.MAX_SEEN) {
      ...
      entries.slice(0, this.MAX_SEEN / 2).forEach((e) =>
        this.seenMessageIds.delete(e),
      );
    }
  }
}
